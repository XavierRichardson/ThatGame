...
namespace ThatGame
{
    class GridSystem
    {
        public void buildBoxList(List<ContentBox> items, int activeBox = 0) {
            ...
            Console.WriteLine((activeBox + 1) + "/" + itemCount);
        ...
        private List<ContentBox> getActiveBoxes(List<ContentBox> items, int activeBox) {
            ...
            if (activeBox < 0) {
                activeBox = 0;
            } else if (activeBox > items.Count) {
                activeBox = items.Count;
            }
